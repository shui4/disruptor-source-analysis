...
public class EventPollerTest {
  ...
  public void shouldPollForEvents() throws Exception {
    ...
    final SingleProducerSequencer sequencer =
        new SingleProducerSequencer(16, new BusySpinWaitStrategy());
    final EventPoller.Handler<Object> handler =
        new EventPoller.Handler<Object>() {
    ...
    final DataProvider<Object> provider =
  ...
  public void shouldSuccessfullyPollWhenBufferIsFull() throws Exception {
    ...
    final EventPoller.Handler<byte[]> handler =
        new EventPoller.Handler<byte[]>() {
    ...
    final RingBuffer<byte[]> ringBuffer =
        RingBuffer.createMultiProducer(factory, 4, new SleepingWaitStrategy());
