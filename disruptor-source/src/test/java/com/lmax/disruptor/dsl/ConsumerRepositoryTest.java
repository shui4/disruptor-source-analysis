...
public class ConsumerRepositoryTest {
  ...
  public void shouldIterateAllEventProcessors() throws Exception {
    ...
    for (ConsumerInfo testEntryEventProcessorInfo : consumerRepository) {
      final EventProcessorInfo<?> eventProcessorInfo =
          (EventProcessorInfo<?>) testEntryEventProcessorInfo;
      if (!seen1
          && eventProcessorInfo.getEventProcessor() == eventProcessor1
          && eventProcessorInfo.getHandler() == handler1) {
        seen1 = true;
      } else if (!seen2
          && eventProcessorInfo.getEventProcessor() == eventProcessor2
