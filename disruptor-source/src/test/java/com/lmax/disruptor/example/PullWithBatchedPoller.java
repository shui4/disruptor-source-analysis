...
class BatchedPoller<T> {
  ...
  private EventPoller.PollState loadNextValues(
      EventPoller<DataEvent<T>> poller, final BatchedData<T> batch) throws Exception {
    return poller.poll(
        new EventPoller.Handler<DataEvent<T>>() {
          @Override
          public boolean onEvent(DataEvent<T> event, long sequence, boolean endOfBatch)
              throws Exception {
